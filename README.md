...
[![Build Status](https://github.com/weicrypto/insight-api/actions/workflows/test_and_release.yml/badge.svg)](https://github.com/weicrypto/insight-api/actions/workflows/test_and_release.yml)
[![NPM version](https://img.shields.io/npm/v/@weicrypto/insight-api.svg)](https://npmjs.org/package/@weicrypto/insight-api)
...
A Wei blockchain REST and WebSocket API Service

This is a backend-only service. If you're looking for the web frontend application, take a look at https://github.com/weicrypto/insight-ui.
...
- [API HTTP Endpoints](#api-http-endpoints)
    ...
    - [Status of the Wei Network](#status-of-the-wei-network)
...
```bash
npm install -g @weicrypto/weicore-node
weicore-node create mynode
cd mynode
weicore-node install @weicrypto/insight-api
weicore-node start  # to also start the service
```
...
- [Weicore Node Wei 4.x](https://github.com/weicrypto/weicore-node)

**Note:** You can use an existing Wei data directory, however `txindex`, `addressindex`, `timestampindex` and `spentindex` need to be enabled in `wei.conf`, as well as a few other additional fields.

### Query Rate Limit

To protect the server, insight-api has a built-in query rate limiter. It can be configurable in `weicore-node.json` with:
...
With all the configuration options available: https://github.com/weicrypto/insight-api/blob/master/lib/ratelimiter.js#L10-17
...
```
{
[comment]: <> (update url dashcentral.org)
...
`<weiAddress>`: new transaction concerning <weiAddress> received from network. This event is published in the `<weiAddress>` room.
...
- The response for the `/sync` endpoint does not include `startTs` and `endTs` as the sync is no longer relevant as indexes are built in weid.
- The endpoint for `/peer` is no longer relevant connection to weid is via ZMQ.
...
The `/status?q=getTxOutSetInfo` method has also been removed due to the query being very slow and locking weid.
...
Feel free to dive in! [Open an issue](https://github.com/weicrypto/insight-api/issues/new) or submit PRs.
